...
Keystatic умеет настроить всё сам. Для этого:

1. Убедитесь, что `PUBLIC_KEYSTATIC_STORAGE=github` выставлен в `.env`
2. Запустите сервер (`npm run dev`) и откройте `http://127.0.0.1:4321/keystatic`
3. Keystatic запустит пошаговый мастер настройки — он сам создаст OAuth App на GitHub с правильными параметрами и выдаст нужные переменные
4. Скопируйте полученные значения в `.env` и перезапустите сервер
